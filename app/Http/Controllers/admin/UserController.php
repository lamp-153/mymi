...
{	
    ...
    {   
    	//1.获取表中所有用户信息
    	$list = \DB::table('store_user')->where('user_type','2')->get();
    	//2.跳转视图，并将用户信息带上
    ...
    {	
    	//1.获取到修改的信息
    	$user = $request->only('user_realname','user_pass','user_email','user_integral','user_sex','user_state');
        //2、判断密码是否相同
    ...
    {   
    	//1.判断要添加的用户信息是否存在
    	if($request->has('user_name','user_realname','user_pass','pass','user_sex','user_email','user_integral','user_state','user_time')){
            //2.获取到要添加的信息
    		$data = $request->only('user_name','user_realname','user_pass','user_sex','user_email','user_integral','user_state','user_time');
            //3、判断用户名是否已经存在
